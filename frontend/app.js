// Base URL del backend (Railway). Vacío = mismo origen. Se define en config.js o en build (VITE_API_URL).
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const [steamRes, nuuRes, fanRes, gmgRes, igRes] = await Promise.allSettled([
      fetch(apiUrl(`/search?${params.toString()}`)).then((r) => (r.ok ? r.json() : r.json().then((e) => { throw e; }))),
      fetch(apiUrl(`/nuuvem?${params.toString()}`)).then((r) => (r.ok ? r.json() : r.json().then((e) => { throw e; }))),
      fetch(apiUrl(`/fanatical?${params.toString()}`)).then((r) => (r.ok ? r.json() : r.json().then((e) => { throw e; }))),
      fetch(apiUrl(`/greenmangaming?${params.toString()}`)).then((r) => (r.ok ? r.json() : r.json().then((e) => { throw e; }))),
      fetch(apiUrl(`/instantgaming?${params.toString()}`)).then((r) => (r.ok ? r.json() : r.json().then((e) => { throw e; }))),
    ]);
