...
with sync_playwright() as p:
  ...
  return matches.slice(0,200);
    ...
    selectors = ['.price','.product-price','.price--now','.current-price','#pdp_price','.price__amount','.sale-price','.price-container','gmg-price','gmgprice']
    ...
    # also try querying for elements containing COP or %
    elements = page.query_selector_all("xpath=//*[contains(text(),'COP') or contains(text(),'%')]")
    if elements:
        print('\nElements with COP or % in text:')
        for e in elements:
            try:
                txt = e.inner_text()
                print('-', txt[:300])
